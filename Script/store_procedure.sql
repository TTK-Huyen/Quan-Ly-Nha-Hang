...
GO
CREATE PROCEDURE THEM_CHI_NHANH @TENCHINHANH NVARCHAR(100), @DIACHI NVARCHAR(255), @THOIGIANMOCUA TIME, @THOIGIANDONGCUA TIME, @SDT VARCHAR(10), @BAIDOXEHOI BIT, @BAIDOXEMAY BIT, @NVQL CHAR(6), @MAKV TINYINT, @GIAOHANG BIT
AS
BEGIN
						...
						BEGIN
							...
							ELSE
								IF(@THOIGIANMOCUA<@THOIGIANDONGCUA)
									BEGIN
										IF EXISTS (SELECT 1 FROM KhuVuc_ThucDon WHERE MaKhuVuc=@MAKV)
											BEGIN
												INSERT INTO ChiNhanh(TenChiNhanh, DiaChi, ThoiGianMoCua, ThoiGianDongCua, SoDienThoai, BaiDoXeHoi, BaiDoXeMay, NhanVienQuanLy, MaKhuVuc, GiaoHang) values (@TENCHINHANH, @DIACHI, @THOIGIANMOCUA, @THOIGIANDONGCUA, @SDT, @BAIDOXEHOI, @BAIDOXEMAY, @NVQL, @MAKV, @GIAOHANG)
											END
										ELSE
											BEGIN
...
AS
BEGIN
	IF EXISTS (SELECT 1 FROM KhuVuc_ThucDon AS KV WHERE KV.MaKhuVuc=@MAKHUVUC)
		BEGIN
...
CREATE PROCEDURE THEM_MON @MAMON TINYINT, @MAMUC TINYINT, @TENMON NVARCHAR(100), @GIAHIENTAI DECIMAL(18,3), @GIAOHANG BIT
AS
BEGIN
...
CREATE PROCEDURE THEMMON_VAOTHUCDON @TENTHUCDON NVARCHAR(100), @MAMON TINYINT, @MAKV TINYINT
AS
BEGIN
	IF EXISTS(SELECT 1 FROM ThucDon AS TD WHERE TD.MaThucDon=@MATHUCDON)
		BEGIN
			IF EXISTS(SELECT 1 FROM Mon AS M WHERE M.MaMon=@MAMON)
				BEGIN
					IF EXISTS(SELECT 1 FROM KhuVuc_ThucDon WHERE @MAKV=MaKhuVuc AND @MAMON=MaMon AND @TENTHUCDON=TenThucDon)
						BEGIN
							RAISERROR(N'Món đã tồn tại trong thực đơn',16,1)
						END
					ELSE
						BEGIN
							INSERT INTO ThucDon_Mon (MaKhuVuc, TenThucDon, MaMon) values (@MAKV, @TENTHUCDON, @MAMON)
						END
				END
			ELSE
				BEGIN
...
CREATE PROCEDURE XOAMON_KHOITHUCDON @MAKV TINYINT, @TENTHUCDON NVARCHAR(100) ,@MAMON TINYINT
AS
BEGIN
	IF EXISTS(SELECT 1 FROM ThucDon AS TD WHERE TD.MaThucDon=@MATHUCDON)
		BEGIN
			IF EXISTS(SELECT 1 FROM Mon AS M WHERE M.MaMon=@MAMON)
				BEGIN
					IF EXISTS (SELECT 1 FROM ThucDon_Mon WHERE @MAKV=MaKhuVuc AND @TENTHUCDON=TenThucDon AND @MAMON=MaMon)
						BEGIN
							DELETE FROM ThucDon_Mon WHERE MaKhuVuc=@MAKHUVUC and TenThucDon=@TENTHUCDON and MaMon=@MAMON
						END
					ELSE
						BEGIN
							RAISERROR(N'Không tìm thấy món trong thực đơn',16,1)
						END
				END
			ELSE
				BEGIN
