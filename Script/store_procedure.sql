...
CREATE PROCEDURE THEMMON_VAOTHUCDON @MAKHUVUC TINYINT,@TENTHUCDON NVARCHAR(100), @MAMON SMALLINT
AS
BEGIN
	IF EXISTS(SELECT 1 FROM KhuVuc_ThucDon AS KT WHERE KT.MaKhuVuc=@MAKHUVUC)
		BEGIN
			IF EXISTS(SELECT 1 FROM Mon AS M WHERE M.MaMon=@MAMON)
				BEGIN
					INSERT INTO ThucDon_Mon (MaKhuVuc, TenThucDon, MaMon) 
					VALUES (@MAKHUVUC , @TENTHUCDON ,@MAMON );
				END
			ELSE
				BEGIN
...
CREATE PROCEDURE XOAMON_KHOITHUCDON @MAKHUVUC TINYINT,@TENTHUCDON NVARCHAR(100), @MAMON SMALLINT
AS
BEGIN
	IF EXISTS(SELECT 1 FROM KhuVuc_ThucDon AS KT WHERE KT.MaKhuVuc=@MAKHUVUC)
		BEGIN
